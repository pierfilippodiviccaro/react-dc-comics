
export default function Main (){
return (
<main>
<div className="maio blue flex text justify align">
    <nav className="flex row gap2 maio none style top">
        <li className="flex"><img src="/img/buy-comics-digital-comics.png" alt="" className="grandezza"/><a href="">miao</a></li>
        <li><img src="/img/buy-comics-merchandise.png" alt="" className="grandezza" /><a href="">miao</a></li>
        <li><img src="/img/buy-comics-shop-locator.png" alt="" className="grandezza" /><a href="">miao</a></li>
        <li><img src="/img/buy-comics-subscriptions.png" alt="" className="grandezza" /><a href="">miao</a></li>
        <li><img src="/img/buy-dc-power-visa.svg" alt="" className="grandezza2" /><a href="">miao</a></li>
    </nav>
</div>
</main>




)
}